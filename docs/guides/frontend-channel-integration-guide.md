# Frontend Channel Integration Guide

> 웹(ai-ott-web)과 어드민(ai-ott-admin) 프론트엔드에서 Creator & Channel 시스템을 연동하기 위한 상세 가이드
...
- [ ] 채널 페이지 라우트 추가 (`/channels/:handle`)
- [ ] 채널 헤더 컴포넌트 (배너, 프로필, 이름, 구독자수, 구독 버튼)
- [ ] `isOfficial === true`이면 공식 채널 뱃지 표시
- [ ] 콘텐츠 그리드 (썸네일, 제목, 타입, 날짜)
- [ ] 무한 스크롤 또는 "더 보기" 페이징 (`offset` 증가)
- [ ] 시리즈 탭 (시리즈 목록 + 에피소드 수)
- [ ] 콘텐츠 클릭 → 기존 재생 페이지로 이동
- [ ] `thumbnailUrl`이 null일 때 기본 썸네일 표시
- [ ] `profileImageUrl`이 null일 때 기본 아바타 표시
- [ ] `bannerImageUrl`이 null일 때 기본 그라데이션 배너
...
- [ ] 구독/구독해제 API 호출 함수
- [ ] 구독 버튼 상태 관리 (구독됨/안됨)
- [ ] 구독 후 subscriberCount 로컬 업데이트 (+1 / -1)
- [ ] 비로그인 시 구독 버튼 → 로그인 페이지로 리다이렉트
- [ ] 내 구독 목록 페이지 (선택사항)

---

### 1.3 크리에이터 스튜디오 (내 채널 관리)
...
- [ ] 크리에이터 스튜디오 라우트 및 레이아웃
- [ ] 채널 설정 페이지 (이름, 설명, 프로필/배너 이미지 수정)
- [ ] 내 콘텐츠 목록 테이블 (상태별 필터링, 정렬)
- [ ] 콘텐츠 생성 폼 (MOVIE / EPISODE 모드 전환)
- [ ] EPISODE 모드: 시리즈 선택 또는 새 시리즈 생성
- [ ] 콘텐츠 상태 변경 (PUBLISHED ↔ UNLISTED)
- [ ] 콘텐츠 삭제 (확인 다이얼로그)
- [ ] 콘텐츠 메타데이터 수정 (제목, 설명)
- [ ] 시리즈 관리 페이지
- [ ] videoAssetStatus에 따른 상태 표시 (업로드중 / 트랜스코딩중 / 준비됨 / 실패)
- [ ] 비디오 파일 업로드 기능 (기존 asset API 연동)

---

### 1.4 기존 페이지에 채널 링크 추가
...
### 2.2 기존 콘텐츠 관리에 채널 정보 표시

기존 Admin 콘텐츠 목록/상세에 채널 정보를 추가로 표시.

> **TODO (서버):** AdminContentSummary, AdminContentDetail에 channelName, channelHandle 필드 추가 필요.

#### 구현 체크리스트

- [ ] (서버 수정 후) 콘텐츠 목록에 채널명 컬럼 추가
- [ ] (서버 수정 후) 콘텐츠 상세에 채널 정보 표시
- [ ] 채널명 클릭 → 채널 관리 상세로 이동
...
| 3 | AdminContentSummary에 channelName 추가 | 중간 |
| 4 | 기존 Admin 업로드(UnifiedUploadService)에 channel_id 통합 (Task 8) | 중간 |
| 5 | 채널 handle 변경 API | 낮음 |
| 6 | 프로필/배너 이미지 업로드 API (또는 presigned URL) | 낮음 |
| 7 | 구독 여부 확인 단일 API (`GET /api/app/channels/{handle}/subscription-status`) | 낮음 |
